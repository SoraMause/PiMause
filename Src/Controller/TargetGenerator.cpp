...
int16_t TargetGenerator::clacSideSensorP(Sensor_Data& sen_left, Sensor_Data& sen_right, bool act, bool rotation)
{
  ...
  if( act && !rotation && (store_velocity > 100.0f) ){
    if(sen_left.now > sen_left.threshold && sen_right.now > sen_right.threshold && sen_left.diff_1ms < 20 && sen_right.diff_1ms < 20){
      // 両壁があるとき
    ...
    } else if(sen_left.now > sen_left.threshold && sen_left.diff_1ms < 10 ){
      // 左壁があるとき
    ...
    } else if(sen_right.now > sen_right.threshold && sen_right.diff_1ms < 10 ){
      // 右壁があるとき
...
int16_t TargetGenerator::calcFrontWallSensor(Sensor_Data& sen_front, bool act, float now_val, bool control)
{
  float step_value = 0.0f;
  if(sen_front.now > 800 && !act && now_val >= 0.0f && control){
